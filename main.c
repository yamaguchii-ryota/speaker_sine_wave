...
// ===== 同期パルス用（RB6）=====
#define SYNC_TRIS TRISBbits.TRISB6
#define SYNC_LAT  LATBbits.LATB6

#define SYNC_PULSE_TICKS 96      // パルス幅 ≒ 2 ms
#define SYNC_PERIODS     5       // 何周期ごとにパルスを出すか（ここを5に）

volatile uint8_t  sync_pulse_counter = 0; // パルス幅用カウンタ
volatile uint16_t sync_cycle_counter = 0; // 正弦波周期カウンタ
...
int main(void) {
    ...
    const float TARGET_FREQUENCY = 120.0f; // 例: 100Hz
 ...
 * ※ タイムテーブル1周期ごとにGPIOピンにパルス信号を送ります。
 */
void __attribute__((__interrupt__, no_auto_psv)) _T1Interrupt(void) {

    // --- 同期パルスの幅制御（毎サンプルごとにカウンタを減らす） ---
    if (sync_pulse_counter > 0) {
        sync_pulse_counter--;
        if (sync_pulse_counter == 0) {
            SYNC_LAT = 0;     // カウンタが0になったら Low に戻す
        }
    }
    ...
    // 4. ループ処理（テーブル一周＝正弦波1周期ごと）
        if (sine_float_index >= current_table_size) {
        sine_float_index -= current_table_size;

        // 正弦波が1周期終わるたびにカウントアップ
        sync_cycle_counter++;

        // SYNC_PERIODS 回たまったらパルスを出す
        if (sync_cycle_counter >= SYNC_PERIODS) {
            sync_cycle_counter = 0;          // カウンタリセット
            SYNC_LAT = 1;                    // パルスON
            sync_pulse_counter = SYNC_PULSE_TICKS;  // この回数分High維持
        }
    }
